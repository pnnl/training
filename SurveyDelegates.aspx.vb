...
#Region " Web Form Designer Generated Code "
    ...
    Private Sub InitializeComponent()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(SurveyDelegates))
    ...
    Protected WithEvents lblSurvey As System.Web.UI.WebControls.Label
...
#Region "Basic"
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Session("isAuthenticated") <> "True") Then
            ...
            Session("pageSel") = "Login"
            redirect(Session("CurrentPage"))
        ElseIf (myUtility.checkUserType(2, CType(Session("isSurveyOwner"), Boolean), CType(Session("isSurveyDelegate"), Boolean))) Then
            redirect(Session("CurrentPage"))
        Else
            Session("CurrentPage") = "./SurveyDelegates.aspx"
        ...
        If Not (IsPostBack) Then
            ...
            If (Session("ProcessStopped") = "False") Then
                'refresh the tables
                Dim oldID As Integer = Session("seqSurveyUserID")
                Session("seqSurveyUserID") = -1
                ...
                Else
                    ...
                    If (isExistingUser) Then
                        ...
                        If (Me.dsCommon.Tables("ExistingUserODStatus").Rows.Count() > 0) Then
                            Session("seqSurveyUserID") = Me.dsCommon.Tables("ExistingUsers").Rows(0).Item("USER_KEY")
                            Session("SurveyDelegateRow") = findRow("Delegates", Session("seqSurveyUserID"))
                        Else
                    ...
                    myUtility.optionPreSet(Session("seqSurveyUserID"), Session("SurveyDelegateMax"), Me.pageOptions)
...
#Region "Data Load"
    ...
    Private Function loadDelegates() As Boolean
        ...
        Try
            ...
            'select only the delegates for the current survey
            ...
            myUtility.getExtension() & "SAT_USER fu, " & myUtility.getExtension() & "SAT_USER_PERMISSION fp Where fu.USER_KEY = fp.USER_KEY and fp.SURVEY_KEY = " & Session("seqSurvey") & _
            ...
            Session("SurveyDelegateMax") = Me.dsCommon.Tables("Delegates").Rows.Count()

            'determine if there is any data and if the user id has been set
            If (Session("SurveyDelegateMax") = 0) Then
                Session("seqSurveyUserID") = -1
            ElseIf (Session("seqSurveyUserID") Is Nothing) Then
                Session("seqSurveyUserID") = 1
            End If

            'make sure the delegate row never exceeds the maximum delegates
            If (Session("SurveyDelegateRow") > Session("SurveyDelegateMax")) Then
                Session("SurveyDelegateRow") = Session("SurveyDelegateMax") - 1
            End If
            ...
            If (Session("seqSurveyUserID") = -1) Then
                Session("SurveyDelegateRow") = Session("SurveyDelegateMax")
            ElseIf Not (Session("SurveyDelegateMax") = Session("SurveyDelegateRow")) Then
                Session("SurveyDelegateRow") = findRow("Delegates", Session("seqSurveyUserID"))
            ElseIf (Session("SurveyDelegateMax") = Session("SurveyDelegateRow") And Session("seqSurveyUserID") <> -1) Then
                Session("seqSurveyUserID") = -1
            End If
            Return True
        Catch ex As Exception
    ...
    Private Function loadUsers() As Boolean
        ...
        Try
            ...
            sqlCommonAction.CommandText = "select USER_KEY from " & myUtility.getExtension() & "SAT_USER_PERMISSION where SURVEY_KEY = " & Session("seqSurvey") & _
...
#Region "Control Settings"
    ...
    Private Sub setControls(Optional ByVal override As String = "")
        ...
        If (Session("seqSurveyUserID") > 0) Then
            Try
                Me.txtEmail.Text = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("EMAIL_ADDRESS")
                Me.txtFirstName.Text = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("FIRST_NAME")
                Me.txtLastName.Text = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("LAST_NAME")
                Me.txtMiddleName.Text = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("MIDDLE_NAME")
                Me.txtSuffixName.Text = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("SUFFIX_NAME")
                Me.txtPhoneNumber.Text = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("PRIMARY_WORK_PHONE")
                Me.txtCompany.Text = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("COMPANY_ABBREV")
                Me.chkTQSurvey.Checked = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("TRAINING_IND")
                Me.chkActive.Checked = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("ACTIVE_IND")
                Me.chkOwner.Checked = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("OWNER_IND")
                Me.lblNavBar.Text = "Delegate " & Session("SurveyDelegateRow") + 1 & " of " & Session("SurveyDelegateMax")
            Catch ex As Exception
                Trace.Warn(ex.ToString)
            End Try
        Else
            If (override = "") Then
                ...
                Me.chkOwner.Checked = 0
            End If
            Me.lblNavBar.Text = "Delegate " & Session("SurveyDelegateRow") + 1 & " of " & Session("SurveyDelegateMax") + 1
        End If
    End Sub
#End Region
...
#Region "Checks"
    ...
    Private Function checkUserExistence(ByVal dr As DataRow) As Boolean
        Try
            ...
            "and fu.MIDDLE_NAME = hp.middle_initial and hp.active_sw = 'Y' and fu.HANFORD_ID = hp.hanford_id and hp.hanford_id = '" & dr.Item("HANFORD_ID") & "'"
    ...
    Private Function checkOwnerDelegate(ByVal dr As DataRow) As Boolean
        Try
            ...
            "fu.USER_KEY = fp.USER_KEY And fp.SURVEY_KEY = " & Session("seqSurvey") & _
...
#Region "Settings"
    ...
    Private Sub setNavBar()
        'to determine what of the nav bar buttons should be available
        myNavBar.wnb_MoveTo(Me.navButtons, Session("SurveyDelegateMax"), Session("SurveyDelegateRow"), Switch)
    End Sub
#End Region
...
#Region "Find"
    ...
    Private Function processHID(ByVal LookUpString As String) As Boolean
        Try
            ...
            "and hp.active_sw = 'Y' and fu.HANFORD_ID = '" & LookUpString & "'" & " and fu.HANFORD_ID <> ''"
            sqlCommonAdapter.Fill(Me.dsCommon, "ExistingUsers")
            ...
            If (alreadyExists) Then
                'if we found something determine if they are already an owner/delegate of the survey
                sqlCommonAction.CommandText = "select OWNER_IND from " & myUtility.getExtension() & "SAT_USER fu, " & _
...
#Region "Nav Bar Events"
    ...
    Private Sub wnbDelegates_MoveToStart(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnStart.Click
        ...
        Else
            Try
                Session("SurveyDelegateRow") = 0
                Session("seqSurveyUserID") = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("USER_KEY")
                setNavBar()
                setControls()
                'set the page options
                myUtility.optionPreSet(Session("seqSurveyUserID"), Session("SurveyDelegateMax"), Me.pageOptions)
            ...
            Catch ex As Exception
                Trace.Warn(ex.ToString)
                alert("Error moving to the first survey delegate/owner.")
            End Try
        End If
    End Sub
    ...
    Private Sub wnbDelegates_MovePrev(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnPrev.Click
        ...
        Else
            Try
                Session("SurveyDelegateRow") -= 1
                If (Session("SurveyDelegateRow") = -1) Then
                    Session("SurveyDelegateRow") = 0
                End If
                Session("seqSurveyUserID") = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("USER_KEY")
                setNavBar()
            ...
            Catch ex As Exception
                Trace.Warn(ex.ToString)
                alert("Error moving to the previous survey delegate/owner.")
            End Try
        End If
    End Sub
    ...
    Private Sub wnbDelegates_MoveReload(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnReload.Click
        ...
        Else
            ...
            Catch ex As Exception
                Trace.Warn(ex.ToString)
                alert("Error resetting the current survey delegate/owner.")
            End Try
        End If
    End Sub
    ...
    Private Sub wnbDelegates_MoveNext(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnNext.Click
        ...
        Else
            ...
            Catch ex As Exception
                Trace.Warn(ex.ToString)
                alert("Error moving to the next survey delegate/owner.")
            End Try
        End If
    End Sub
    ...
    Private Sub wnbDelegates_MoveToEnd(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnLast.Click
        ...
        Else
            ...
            Catch ex As Exception
                Trace.Warn(ex.ToString)
                alert("Error moving to the last survey delegate/owner.")
            End Try
        End If
    End Sub
    ...
    Private Sub wnbDelegates_InsertRow(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnNew.Click
        ...
        Else
            Try
                ...
                Session("SurveyDelegateRow") = Session("SurveyDelegateMax")
                Me.dsCommon.Tables("Delegates").Rows.Add(Me.dsCommon.Tables("Delegates").NewRow())
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("EMAIL_ADDRESS") = ""
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("FIRST_NAME") = ""
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("LAST_NAME") = ""
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("MIDDLE_NAME") = ""
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("SUFFIX_NAME") = ""
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("PRIMARY_WORK_PHONE") = ""
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("COMPANY_ABBREV") = ""
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("USER_CODE") = -1
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("TRAINING_IND") = 0
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("ACTIVE_IND") = 1
                Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateMax")).Item("OWNER_IND") = 0
                myNavBar.wnb_MoveTo(Me.navButtons, Session("SurveyDelegateMax"), Session("SurveyDelegateRow"))
                setControls()
                'set the page options
                myUtility.optionPreSet(Session("seqSurveyUserID"), Session("SurveyDelegateMax"), Me.pageOptions)
            ...
            Catch ex As Exception
                Trace.Warn(ex.ToString)
                alert("Error creating new survey delegate/owner.")
            End Try
        End If
    End Sub
#End Region
...
#Region "Page Action"
        ...
        Try
            ...
            If (loadData()) Then
                ...
                If (userRow <> -1) Then
                    ...
                    Session("seqSurveyUserID") = userID
                    ...
                    If (Me.txtHIDPayrollName.Text <> "") Then
                        ...
                        If (getHanfordPeopleInfo(Me.txtHIDPayrollName.Text)) Then
                            ...
                            If (Me.dsCommon.Tables.Contains("ExistingUserODStatus")) Then
                                If (Me.dsCommon.Tables("ExistingUserODStatus").Rows.Count() > 0) Then
                                    Me.lblNavBar.Text = "Survey " & Session("SurveyDelegateRow") + 1 & " of " & Session("SurveyDelegateMax")
                                End If
                            Else
                                Me.lblNavBar.Text = "Survey " & Session("SurveyDelegateRow") + 1 & " of " & Session("SurveyDelegateMax") + 1
                            End If
    ...
    Private Sub btnInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnInsert.Click
        ...
        If (blnContinue) Then
            ...
            'check the user type
            If (myUtility.checkUserType(2, CType(Session("isSurveyOwner"), Boolean), CType(Session("isSurveyDelegate"), Boolean))) Then
                Response.Redirect(Session("CurrentPage"))
            End If
...
#Region "Insert Code"
        ...
        Try
            ...
            If (myUtility.setupTransaction(sqlCommonAction, Me.commonConn)) Then
                ...
                If (Me.txtFirstName.Text <> "" And Me.txtLastName.Text <> "" And Me.txtEmail.Text <> "") Then
                        ...
                        If (insertPermission(sqlCommonAction, sqlCommonAdapter)) Then
                            ...
                            If (Me.dsCommon.Tables("UserList").Rows.Count <> 0) Then
                                If Not (sendMail(Me.txtEmail.Text, Session("newUserName"), True)) Then
                                    alert("Failed to notify the new Delegate/Owner of this survey via e-mail.")
                                End If
                            Else
                    ...
                    End If
                Else
                    alert("You must find a person to insert and select a user type at a minimum.")
                    failure = True
                    sqlCommonAction.Transaction.Rollback()
                End If
                Return failure
    ...
    Private Function insertUser(ByVal sqlCommonAction As System.Data.SqlClient.SqlCommand, ByVal sqlCommonAdapter As System.Data.SqlClient.SqlDataAdapter) As Boolean
        Try
            ...
            If (Me.dsCommon.Tables("UserList").Rows.Count = 1) Then
                ...
                If (Me.dsCommon.Tables("UserList").Rows(0).Item("USER_CODE") < 2) Then
                    sqlCommonAction.CommandText = "Update " & myUtility.getExtension() & "SAT_USER set USER_CODE = 2 where USER_KEY = " & _
                ...
                Session("seqSurveyUserID") = Me.dsCommon.Tables("UserList").Rows(0).Item("USER_KEY")
                Return True
            ...
            "TRAINING_IND, COMPANY_ABBREV, NO_EMAIL_IND) VALUES (@HANFORD_ID, @EMAIL_ADDRESS, 2, @LAST_NAME," & _
            ...
            Session("seqSurveyUserID") = Me.dsCommon.Tables("UserIDFetch").Rows(0).Item("USER_KEY")
            Return True
        Catch ex As Exception
            Trace.Warn(ex.ToString)
            Return False
    ...
    Private Function insertPermission(ByVal sqlCommonAction As System.Data.SqlClient.SqlCommand, ByVal sqlCommonAdapter As System.Data.SqlClient.SqlDataAdapter) As Boolean
        Try
            ...
            'determine if the user already exists for the survey
            sqlCommonAction.CommandText = "Select * from " & myUtility.getExtension() & "SAT_USER_PERMISSION where USER_KEY = " & Session("seqSurveyUserID") & _
            " and SURVEY_KEY = " & Session("seqSurvey")
            sqlCommonAdapter.Fill(Me.dsCommon, "PermissionList")

            'if the user already exists under the survey then update their record for that survey
            If (Me.dsCommon.Tables("PermissionList").Rows.Count() > 0) Then
                ...
                sqlCommonAction.CommandText &= " where USER_KEY = " & Session("seqSurveyUserID") & " and SURVEY_KEY = " & Session("seqSurvey")
            Else
                If (Me.dsCommon.Tables("ExistingDelegates").Rows.Count() > 0) Then
                    ...
                    sqlCommonAction.CommandText = "Insert INTO " & myUtility.getExtension() & "SAT_USER_PERMISSION (USER_KEY, TEMPLATE_KEY, SURVEY_KEY, CREATOR_USER_KEY, " & _
                        "CREATE_DATE, OWNER_IND, DELEGATE_IND, USER_IND) VALUES (" & Session("seqSurveyUserID") & ", -1, " & Session("seqSurvey") & _
                        ", " & Session("UserID") & ", '" & Now & "', " & IIf(Me.chkOwner.Checked, 1, 0) & ", 1, 1)"
                Else
                    'Add permissions for the survey
                    sqlCommonAction.CommandText = "Insert INTO " & myUtility.getExtension() & "SAT_UER_PERMISSION (USER_KEY, TEMPLATE_KEY, SURVEY_KEY, CREATOR_USER_KEY, " & _
                        "CREATE_DATE, OWNER_IND, DELEGATE_IND, USER_IND) VALUES (" & Session("seqSurveyUserID") & ", -1, " & Session("seqSurvey") & _
                        ", " & Session("UserID") & ", '" & Now & "', 1, 1, 1)"
                End If
            End If
            ...
            'determine if the user already exists for the Template
            sqlCommonAction.CommandText = "Select * from " & myUtility.getExtension() & "SAT_USER_PERMISSION where USER_KEY = " & _
            Session("seqSurveyUserID") & " and TEMPLATE_KEY = " & Session("seqTemplate")
            Me.dsCommon.Tables("PermissionList").Rows.Clear()
            sqlCommonAdapter.Fill(Me.dsCommon, "PermissionList")

            'add the user as a user of the parent template
            If (Me.dsCommon.Tables("PermissionList").Rows.Count() <= 0) Then
                sqlCommonAction.CommandText = "Insert INTO " & myUtility.getExtension() & "SAT_USER_PERMISSION (USER_KEY, TEMPLATE_KEY, " & _
                "SURVEY_KEY, CREATOR_USER_KEY, CREATE_DATE, OWNER_IND, DELEGATE_IND, USER_IND) VALUES " & _
                "(" & Session("seqSurveyUserID") & ", " & Session("seqTemplate") & ", -1, " & _
                Session("UserID") & ", '" & Now & "', 0, 0, 1)"
        ...
        Try
            ...
            If (myUtility.setupTransaction(sqlCommonAction, Me.commonConn)) Then
                ...
                If (Me.txtFirstName.Text <> "" And Me.txtLastName.Text <> "" And Me.txtEmail.Text <> "") Then
                        ...
                        If (updatePermission(sqlCommonAction, sqlCommonAdapter)) Then
                            ...
                            If (Session("seqNewOwnerID") >= 1) Then
                                If Not (sendMail(Session("newOwnerEmail"), "Update", True)) Then
                                    alert("Failed to notify the new Owner/Delegate of this survey via e-mail.")
                                End If
                                If Not (sendMail(Me.txtEmail.Text, "Update", True)) Then
                                    alert("Failed to notify the relieved Owner of this survey via e-mail.")
                                End If
                            Else
                                If Not (sendMail(Me.txtEmail.Text, "Update", True)) Then
                                    alert("Failed to notify the Owner/Delegate of this survey via e-mail.")
                                End If
                            End If
                            'determine if the modifed user is the current user and go back to the
                            'template page if the user is no longer an owner
                            If (Session("UserID") = Session("seqSurveyUserID") And Me.chkOwner.Checked = False And Session("UserType") <> 4) Then
                                Session("CurrentPage") = "./Survey.aspx"
                                Session("pageSel") = "Template"
                                redirect(Session("CurrentPage"))
                            End If
                    ...
                    Else
                        sqlCommonAction.Transaction.Rollback()
                        If Not (Session("seqSurveyUserID") = Session("seqNewOwnerID") Or Session("seqNewOwnerID") = -1) Then
                            alert("Failed to update this Owner/Delegate.")
                            failure = True
                        End If
                    End If
    ...
    Private Function updateUser(ByVal sqlCommonAction As System.Data.SqlClient.SqlCommand, ByVal sqlCommonAdapter As System.Data.SqlClient.SqlDataAdapter) As Boolean
        ...
        Try
            ...
            If (Me.chkOwner.Checked = False) Then
                ...
                sqlCommonAction.CommandText = "Select * from " & myUtility.getExtension() & "SAT_USER_PERMISSION fp, " & myUtility.getExtension() & "SAT_USER fu where fp.SURVEY_KEY = " & _
                Session("seqSurvey") & " and (fp.DELEGATE_IND = 1 or fp.OWNER_IND = 1) and fp.USER_KEY = fu.USER_KEY"
                sqlCommonAdapter.SelectCommand = sqlCommonAction
                ...
                If (Me.dsCommon.Tables("TempTable").Rows.Count() > 1) Then
                    ...
                    For Each dr In Me.dsCommon.Tables("TempTable").Rows
                        If (dr.Item("seqUserID") = CType(Session("seqSurveyUserID"), Integer) And dr.Item("OWNER_IND")) Then
                            isOwner = True
                        End If
                    Next
                    ...
                    If (isOwner) Then
                        For Each dr In Me.dsCommon.Tables("TempTable").Rows
                            If (dr.Item("OWNER_IND") And dr.Item("USER_KEY") <> Session("seqSurveyUserID")) Then
                        ...
                        If (Session("seqSurveyUserID") = Session("seqNewOwnerID")) Then
            ...
            If (Session("seqNewOwnerID") <> -1) Then
                ...
                sqlCommonAction.CommandText = "select * from " & myUtility.getExtension() & "SAT_USER_PERMISSION where USER_KEY = " & _
                    Session("seqSurveyUserID") & " and (OWNER_IND = 1 or DELEGATE_IND = 1) and TEMPLATE_KEY <> -1"
                ...
                " where USER_KEY = " & Session("seqSurveyUserID")
    ...
    Private Function updatePermission(ByVal sqlCommonAction As System.Data.SqlClient.SqlCommand, ByVal sqlCommonAdapter As System.Data.SqlClient.SqlDataAdapter) As Boolean
        ...
        If (Session("seqNewOwnerID") >= 1) Then
            Try
                ...
                " where USER_KEY = " & Session("seqNewOwnerID") & " and SURVEY_KEY = " & Session("seqSurvey")

                'do the update
                sqlCommonAction.ExecuteNonQuery()

                'if the user being modified is also the user using this page then update their permission
                If (Session("seqSurveyUserID") = Session("UserID") And Session("seqNewOwnerID") <> Session("UserID") And Session("UserType") <> 4) Then
                    Session("isSurveyOwner") = IIf(Me.chkOwner.Checked, True, False)
                End If
                ...
                If (Session("seqSurveyUserID") <> Session("seqNewOwnerID")) Then
                    ...
                    " where USER_KEY = " & Session("seqSurveyUserID") & " and SURVEY_KEY = " & Session("seqSurvey")
        ...
        Else
            ...
            Try
                'try to insert the user's record in the permission table.
                sqlCommonAction.CommandText = "Insert INTO " & myUtility.getExtension() & "SAT_USER_PERMISSION (USER_KEY, TEMPLATE_KEY, " & _
                "SURVEY_KEY, CREATOR_USER_KEY, CREATE_DATE, OWNER_IND, DELEGATE_IND, USER_IND) VALUES (" & _
                Session("seqSurveyUserID") & ", -1, " & Session("seqSurvey") & ", " & Session("UserID") & _
                ", '" & Now & "', " & IIf(Me.chkOwner.Checked, 1, 0) & ", 1, 1)"
            ...
            If (blnFailure) Then
                Try
                    'do the update if the insert fails
                    sqlCommonAction.CommandText = "Update " & myUtility.getExtension() & "SAT_USER_PERMISSION SET DELEGATE_IND = 1, OWNER_IND = " & _
                    IIf(Me.chkOwner.Checked, 1, 0) & ", USER_IND = 1 where USER_KEY = " & Session("seqSurveyUserID") & " and SURVEY_KEY = " & Session("seqSurvey")
            ...
            'if the user being modified is also the user using this page then update their permission
            If (Session("seqSurveyUserID") = Session("UserID") And Session("UserType") <> 4) Then
                Session("isSurveyOwner") = IIf(Me.chkOwner.Checked, True, False)
            End If
            Return True
        End If
            ...
            If (myUtility.setupTransaction(sqlCommonAction, Me.commonConn)) Then
                ...
                If (deletePermission(sqlCommonAction, sqlCommonAdapter)) Then
                    ...
                    If (Session("seqSurveyUserID") = Session("UserID") And Session("UserType") <> 4) Then
                        ...
                        If (Session("isTemplateUser") = False) Then
                    ...
                    If (Session("SurveyDelegateMax") = 1) Then
                        Session("SurveyDelegateRow") = 0
                        Session("seqSurveyUserID") = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("USER_KEY")
                    ElseIf ((Session("SurveyDelegateRow") + 1) = Session("SurveyDelegateMax")) Then
                        Session("SurveyDelegateMax") -= 1
                        Session("SurveyDelegateRow") -= 1
                        Session("seqSurveyUserID") = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow")).Item("USER_KEY")
                    Else
                        Session("SurveyDelegateMax") -= 1
                        Session("seqSurveyUserID") = Me.dsCommon.Tables("Delegates").Rows(Session("SurveyDelegateRow") + 1).Item("USER_KEY")
                    End If
    ...
    Private Function deletePermission(ByVal sqlCommonAction As System.Data.SqlClient.SqlCommand, ByVal sqlCommonAdapter As System.Data.SqlClient.SqlDataAdapter) As Boolean
        Try
            ...
            For Each row In Me.dsCommon.Tables("Delegates").Rows()
                If (CType(row.Item("USER_KEY"), String) = Session("seqSurveyUserID")) Then
                    If (row.Item("OWNER_IND")) Then
                        'determine if the user being deleted is the only owner
                        sqlCommonAction.CommandText = "Select * from " & myUtility.getExtension() & "SAT_USER_PERMISSION where SURVEY_KEY = " & Session("seqSurvey") & _
                        " and OWNER_IND = 1"
                        sqlCommonAdapter.SelectCommand = sqlCommonAction
                        sqlCommonAdapter.Fill(Me.dsCommon, "OwnersCount")
                        If (Me.dsCommon.Tables("OwnersCount").Rows.Count() < 2) Then
                            alert("You may not delete the only remaining Owner/Delegate.  You must make someone else an owner of this survey before removing this owner.")
                            Return False
                        End If
                    End If
                End If
            ...
            sqlCommonAction.CommandText = "Delete from " & myUtility.getExtension() & "SAT_USER_PERMISSION where SURVEY_KEY = " & Session("seqSurvey") & _
            " and USER_KEY = " & Session("seqSurveyUserID")
            sqlCommonAction.ExecuteNonQuery()
            ...
            'determine if the person being deleted owns or is a delegate of anything else in the template
            sqlCommonAdapter.SelectCommand = sqlCommonAction
            sqlCommonAction.CommandText = "Select * from " & myUtility.getExtension() & "SAT_USER_PERMISSION fp, " & myUtility.getExtension() & "SAT_TEMPLATE_SURVEY fs where " & _
            "fp.SURVEY_KEY = fs.SURVEY_KEY and fs.TEMPLATE_KEY = " & Session("seqTemplate") & _
            " and fp.USER_KEY = " & Session("seqSurveyUserID")
            sqlCommonAdapter.Fill(Me.dsCommon, "Keeper")
            sqlCommonAction.CommandText = "Select * from " & myUtility.getExtension() & "SAT_USER_PERMISSION where TEMPLATE_KEY = " & _
            Session("seqTemplate") & " and USER_KEY = " & Session("seqSurveyUserID") & " and " & _
            "(OWNER_IND = 1 or DELEGATE_IND = 1)"
            sqlCommonAdapter.Fill(Me.dsCommon, "Keeper2")

            'if there are no records remove their access to the template as well
            If (Me.dsCommon.Tables("Keeper").Rows.Count() = 0 And Me.dsCommon.Tables("Keeper2").Rows.Count() = 0) Then
                sqlCommonAction.CommandText = "Delete from " & myUtility.getExtension() & "SAT_USER_PERMISSION where TEMPLATE_KEY = " & _
                Session("seqTemplate") & " and USER_KEY = " & Session("seqSurveyUserID")
                sqlCommonAction.ExecuteNonQuery()
            End If
...
#Region "Reset Code"
    ...
    Private Function doReset() As Boolean
        ...
        Else
            ...
            Catch ex As Exception
                Trace.Warn(ex.ToString)
                alert("Error resetting the delegate/owner of this survey.")
                Return True
            End Try
        End If
    End Function
...
#Region "Send Mail"
    ...
    Private Function sendMail(ByVal Email As String, ByVal strUserName As String, Optional ByVal inSystem As Boolean = False) As Boolean
        ...
        Try
            ...
            Dim strbldrSubject As New StringBuilder("Survey Admin Tool Survey Delegation.")
            ...
            If (Me.dsCommon.Tables("ModifyID").Rows.Count < 1) Then
                alert("Critical error sending e-mail to modified user.  Unable to locate your information.  You may have been removed from the database in the last few minutes.  Contact the Survey Administrator immediately.")
                Return False
            End If
            ...
            If (strUserName = "Delete") Then
                If (Me.chkOwner.Checked = True) Then
                    strbldrMessage.Append("You have been removed as a Owner of " & Session("SurveyName") & _
                         " survey, derived from the " & Session("TemplateName") & " template, by")
                Else
                    strbldrMessage.Append("You have been removed as a Delegate of " & Session("SurveyName") & _
                         " survey, derived from the " & Session("TemplateName") & " template, by")
                End If
            ...
            ElseIf (strUserName = "Update") Then
                ...
                If (Me.chkOwner.Checked = True) Then
                    ...
                    strbldrMessage.Append(Session("SurveyName") & " survey, derived from the " & Session("TemplateName") & _
                        " template, or your user information as a Owner has been altered by")
                Else
